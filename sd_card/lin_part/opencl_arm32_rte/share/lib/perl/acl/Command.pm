...
sub version {
   my ($self,@args) = @_;
   if ( $#args < 0 ) {
      my $banner = acl::Env::is_sdk() ? 'Altera SDK for OpenCL, Version 14.1.0 Build 180, Copyright (C) 2014 Altera Corporation' : 'Altera Runtime Environment for OpenCL, Version 14.1.0 Build 180, Copyright (C) 2014 Altera Corporation';
      print $self->prog." 14.1.0.180 ($banner)\n";
   } else {
...
sub report {
   ...
   if ( defined $aocx && defined $mon ) {
	 ...
	 return $self;
      ...
      my $command = aoclwhich( "$QUARTUS_ROOT","$java_runtime" );
...
sub reprogram {
   ...
   my $command = aoclwhich( "$utilbin","$util" );
   if ( defined $command ) {
